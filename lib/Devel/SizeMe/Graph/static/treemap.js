  ...
  var tm = new $jit.TM.Squarified({
    ...
    Events: {
      ...
      onClick: function(node) {
        if(node) {
            ...
            window.history.pushState({ "id":node.id }, "", "/"+node.id);
            tm.enter(node);
        }
      },
    ...
    request: function(nodeId, level, onComplete){  
            request_jit_tree(nodeId, level, levelsToShow, function(data) {
                ...
                update_node_path(data.node_path);
                // XXX jQuery("#sizeme_path_ul").html(data.node_path_html);
...
function update_node_path(node_path){
  ...
  request_jit_tree_for_node_and_refresh(1); // XXX get from url
    ...
    $jit.util.addEvent($jit.id('goto_parent'), 'click', function() {
        ...
        window.history.pushState({ "id":parent_id }, "", "/"+parent_id);
